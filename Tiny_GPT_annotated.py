...
class TinyGPT(nn.Module):
    def __init__(self, vocab_size, block_size, emb_dim=128, num_heads=4, num_layers=4, dropout=0.1): # num_layers=4 -> block 네개 통과
        ...
        self.lm_head = nn.Linear(emb_dim, vocab_size)      # 128차원 특징을 65개의 문자 중 하나로 바꿈
    ...
    def forward(self, x):
        ...
        h = self.ln_f(h)
        logits = self.lm_head(h)                           # (B,T,128)->(B,T,65) shape 바뀜
...
def sequence_cross_entropy(logits, targets):                                    # cross_entropy: 모델이 정답 문자를 얼마나 잘 맞췄는지를 측정하는 함수
    return F.cross_entropy(logits.transpose(1, 2), targets)                     # transpose: (64,64,65)->(64,65,64)
def train_one_epoch(model, loader, optimizer, device, max_steps=None):          # 한 epoch동안 학습하는 함수
    ...
    for step, (xb, yb) in enumerate(loader):                                    # 매 step마다 새로운 batch를 가져옴
        ...
        logits = model(xb)                                                      # 모델 실행 (64,64,65)
        ...
        if max_steps is not None and step + 1 >= max_steps:                     # 학습시간 줄이기 위해 max step 정해놓음. 300번째 batch까지만 학습
            break
...
for epoch in range(100):                                                        # 100번 반복학습
    train_loss = train_one_epoch(model, loader, optimizer, device, max_steps=300) # 한 epoch 학습 후 평균 loss 반환
